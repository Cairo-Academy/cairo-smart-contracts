...
trait ContractInterface<TContractState> {
    ...
    fn get_signers(self: @TContractState) -> Span<ContractAddress>;
    ...
    fn hash_transaction_batch(self: @TContractState, calls: Span<Call>, salt: felt252) -> TransactionID;

    // Write functions
    fn add_signers(ref self: TContractState, new_quorum: u32, signers_to_add: Span<ContractAddress>);
    fn remove_signers(ref self: TContractState, new_quorum: u32, signers_to_remove: Span<ContractAddress>);
    fn replace_signer(
    ...
    fn submit_transaction_batch(
        ref self: TContractState, calls: Span<Call>, salt: felt252,
    ) -> TransactionID;
    ...
    fn execute_transaction_batch(ref self: TContractState, calls: Span<Call>, salt: felt252);
}
...
mod MultisigWallet {
    ...
    use super::{Errors, SignersInfo, TransactionID, TransactionState, TxInfo, ContractInterface};
    use super::Events::*;
    use crate::src::call_impls::{CallPartialEq, HashCallImpl, HashCallsImpl};
    ...
    fn constructor(ref self: ContractState, quorum: u32, signers: Span<ContractAddress>) {
        self.initializer(quorum, signers);
    }

    #[abi(embed_v0)]
    pub impl MultiSig of ContractInterface<ContractState> {
        ...
        fn get_signers(self: @ContractState) -> Span<ContractAddress> {
        ...
        fn is_confirmed_by(
            self: @ContractState, id: TransactionID, signer: ContractAddress,
        ) -> bool {
        ...
        fn hash_transaction_batch(
            self: @ContractState, calls: Span<Call>, salt: felt252,
        ) -> TransactionID {
        ...
        /// Emits a `CallSalt` event if `salt` is not zero.
        fn submit_transaction_batch(
            ref self: ContractState, calls: Span<Call>, salt: felt252,
        ) -> TransactionID {
    ...
    pub impl InternalImpl of InternalTrait {
        ...
        fn initializer(
            ref self: ContractState, quorum: u32, signers: Span<ContractAddress>,
        ) {
        ...
        ///
        /// - The `caller` must be a registered signer.
        fn assert_one_of_signers(self: @ContractState, caller: ContractAddress) {
        ...
        fn assert_tx_exists(self: @ContractState, id: TransactionID) {
        ...
        ///
        /// Emits a `QuorumUpdated` event if the quorum changes.
        fn _change_quorum(ref self: ContractState, new_quorum: u32) {
            let SignersInfo {
